...
const double max_velocity_scaling_factor =
    0.3; // [move_group_interface] default is 0.1
const double max_acceleration_scaling_factor =
    0.1; // [move_group_interface] default is 0.1

XArmPlanner::XArmPlanner(const rclcpp::Node::SharedPtr &node,
...
bool XArmPlanner::planJointTarget(const std::vector<double> &joint_target) {
  bool success = move_group->setJointValueTarget(joint_target);
  if (!success)
    RCLCPP_WARN(node_->get_logger(), "setJointValueTarget: out of bounds");
  success =
      (move_group->plan(xarm_plan) == moveit::core::MoveItErrorCode::SUCCESS);
  if (!success)
...
bool XArmPlanner::planPoseTarget(const geometry_msgs::msg::Pose &pose_target) {
  bool success = move_group->setPoseTarget(pose_target);
  if (!success)
...
bool XArmPlanner::planPoseTargets(
    const std::vector<geometry_msgs::msg::Pose> &pose_target_vector) {
  bool success = move_group->setPoseTargets(pose_target_vector);
  if (!success)
...
bool XArmPlanner::planCartesianPath(
    const std::vector<geometry_msgs::msg::Pose> &pose_target_vector) {
  // moveit_msgs::msg::RobotTrajectory trajectory;

  double fraction = move_group->computeCartesianPath(
      pose_target_vector, eef_step, jump_threshold, trajectory_);
  bool success = true;
  if (fraction < 0.9) {
    RCLCPP_ERROR(node_->get_logger(),
                 "planCartesianPath: plan failed, fraction=%lf", fraction);
    return false;
  }
  is_trajectory_ = true;
  // https://github.com/ros-planning/moveit2/commit/8bfe782d6254997d185644fa3eb358d2b79d69b2
  // (struct Plan) trajectory_ => trajectory
  // xarm_plan.trajectory_ = trajectory;
  return true;
}
...
bool XArmPlanner::executePath(bool wait) {
  ...
  if (wait)
    code = is_trajectory_ ? move_group->execute(trajectory_)
                          : move_group->execute(xarm_plan);
  else
    code = is_trajectory_ ? move_group->asyncExecute(trajectory_)
                          : move_group->asyncExecute(xarm_plan);
